...
export class AuthService {
    ...
    async signIn(
        ...
        // 입력한 이메일을 사용하는 유저 조회
        const user = await this.usersRepository.findOneAll({ userEmail: signInDto.userEmail });
        ...
        const passwordMatch = await bcrypt.compare(signInDto.userPassword, user['userPassword']);
        if (!passwordMatch) {
            throw new UnauthorizedException("아이디 또는 비밀번호가 일치하지 않습니다.");
        }
